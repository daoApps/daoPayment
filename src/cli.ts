  ...
  .addCommand(
    program.command('create')
      ...
      .action(() => {
        ...
        console.log(`Recovery seed: ${manager.generateRecoverySeed(walletAddress)}`);
      })
  )
  .addCommand(
  ...
  .addCommand(
    program.command('balance')
      ...
      .action(async (address: string) => {
        const balance = await manager.getAgentInterface().getWalletBalance(address);
        console.log(`Balance for ${address}: ${balance} USDC`);
      })
  );

// 权限管理命令
program.command('permission')
  ...
  .addCommand(
    program.command('create')
      ...
      .action((agentId: string, walletAddress: string, options: any) => {
        const permissions = options.permissions ? options.permissions.split(',') : ['transfer'];
        const expiration = options.expiration ? parseInt(options.expiration) : Date.now() + 24 * 60 * 60 * 1000;
        const maxAmount = options.maxAmount ? parseFloat(options.maxAmount) : 100;
        const dailyLimit = options.dailyLimit ? parseFloat(options.dailyLimit) : 500;
  ...
  .addCommand(
    program.command('revoke')
      ...
      .action((permissionId: string) => {
        ...
        console.log(result ? 'Permission revoked successfully' : 'Failed to revoke permission');
      })
  );
  ...
  .addCommand(
    program.command('generate')
      ...
      .action((agentId: string, walletAddress: string, options: any) => {
        ...
        const dailyLimit = options.dailyLimit ? parseFloat(options.dailyLimit) : 20;
  ...
  .addCommand(
    program.command('revoke')
      ...
      .action((sessionKeyId: string) => {
        ...
        console.log(result ? 'Session key revoked successfully' : 'Failed to revoke session key');
      })
  );
  ...
  .addCommand(
    program.command('create')
      ...
      .action((id: string, name: string, description: string) => {
        // 创建默认策略
        const policy = manager.getSecurityManager().createTemplatePolicy('default');
...
program.command('pay')
  ...
  .action(async (agentId: string, walletAddress: string, recipient: string, amount: string, options: any) => {
    ...
    if (options.sessionKey) {
      ...
      // 使用普通权限执行支付
      const result = await manager.getAgentInterface().executePayment(paymentRequest);
      console.log('Payment result:', result);
    }
  });
  ...
  .addCommand(
    program.command('list')
      ...
      .action((options: any) => {
        ...
        if (options.walletAddress) filters.walletAddress = options.walletAddress;
  ...
  .addCommand(
    program.command('report')
      ...
      .action((startTime: string, endTime: string) => {
        const report = manager.getAuditIntegrator().generateAuditReport(
          parseInt(startTime),
          parseInt(endTime)
        );
