...
export function BudgetManager() {
  ...
  const budget = activePolicy
    ? {
        dailyLimit: Number(activePolicy.maxDailySpend),
        weeklyLimit: Number(activePolicy.maxDailySpend) * 7, // Mocking weekly limit
      }
    : undefined;

  const dailySpent = 0; // Mocking spent amount
  const weeklySpent = 0;

  const dailyPercent = budget && budget.dailyLimit > 0
    ? Math.min((dailySpent / budget.dailyLimit) * 100, 100)
    : 0;
  const weeklyPercent = budget && budget.weeklyLimit > 0
