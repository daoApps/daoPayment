...
import useWhitelistRead, {
  useIsRecipientAllowed,
} from '@/src/hooks/useWhitelistRead';

const CONTRACT_ADDRESS =
  (process.env.NEXT_PUBLIC_CONTRACT_ADDRESS as `0x${string}`) ||
  '0x0000000000000000000000000000000000000000';
const DEFAULT_WHITELIST_ID =
...
export function WhitelistManager() {
  ...
  const { data, isLoading } = useWhitelistRead({
    contractAddress: CONTRACT_ADDRESS,
    whitelistId: DEFAULT_WHITELIST_ID,
  });

  const whitelist: WhitelistItem[] =
    data?.recipients.map((addr, idx) => ({
      address: addr,
      category: data.categories[idx] || 'other',
    })) || [];

  const addAddress = async (address: `0x${string}`, category: string) => {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    e.preventDefault();
    await addAddress(newAddress as `0x${string}`, newCategory);
    setNewAddress('');
    setShowAdd(false);
  };
